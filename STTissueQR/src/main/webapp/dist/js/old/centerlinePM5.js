...
function setGradeValue(data){
	...
	//New Code Added Here
	table.find('input[name=basisweighttarget]').parent().prev().text(data.basisweighttarget);
	table.find('input[name=moisturetarget]').parent().prev().text(data.moisturetarget);
	table.find('input[name=yankeemapflow]').parent().prev().text(data.yankeemapflow);
	table.find('input[name=machinechestpumpspeed]').parent().prev().text(data.machinechestpumpspeed);
	...
	//New Code Added Here
	table.find('input[name=SPRLoadingFront]').parent().prev().text(data.sprloadingfront);
	table.find('input[name=SPRLoadingBack]').parent().prev().text(data.sprloadingback);
	table.find('input[name=PickUpRollVacuum]').parent().prev().text(data.pickuprollvacuum);
	table.find('input[name=UhleBoxVacuum]').parent().prev().text(data.uhleboxvacuum);
	table.find('input[name=SPRVacuum]').parent().prev().text(data.sprvacuum);
	table.find('input[name=PrimaryScreenRejectFlow]').parent().prev().text(data.primaryscreenrejectflow);
	table.find('input[name=BlendChestCy]').parent().prev().text(data.blendchestcy);
	table.find('input[name=Refiner1Power]').parent().prev().text(data.refiner1power);
	table.find('input[name=Refiner2Power]').parent().prev().text(data.refiner2power);
	table.find('input[name=Refiner1InletCy]').parent().prev().text(data.refiner1inletcy);
	table.find('input[name=Refiner2InletCy]').parent().prev().text(data.refiner2inletcy);
...
//	table.find('input[name=revision]').val(data.revision);
	...
	//New Fields Add By RLT
	
	table.find('input[name=SPRLoadingFront]').val(data.SPRLoadingFront);
	table.find('input[name=SPRLoadingBack]').val(data.SPRLoadingBack);
	table.find('input[name=PickUpRollVacuum]').val(data.PickUpRollVacuum);
	table.find('input[name=UhleBoxVacuum]').val(data.UhleBoxVacuum);
	table.find('input[name=SPRVacuum]').val(data.SPRVacuum);
	table.find('input[name=PrimaryScreenRejectFlow]').val(data.PrimaryScreenRejectFlow);
	table.find('input[name=BlendChestCy]').val(data.BlendChestCy);
	table.find('input[name=Refiner1Power]').val(data.Refiner1Power);
	table.find('input[name=Refiner2Power]').val(data.Refiner2Power);
	table.find('input[name=Refiner1InletCy]').val(data.Refiner1InletCy);
	table.find('input[name=Refiner2InletCy]').val(data.Refiner2InletCy);
...
function saveCenterline(){
	...
	var SPRLoadingFront=table.find('input[name=SPRLoadingFront]').val();
	var SPRLoadingBack=table.find('input[name=SPRLoadingBack]').val();
	var PickUpRollVacuum=table.find('input[name=PickUpRollVacuum]').val();
	var UhleBoxVacuum=table.find('input[name=UhleBoxVacuum]').val();
	var SPRVacuum=table.find('input[name=SPRVacuum]').val();
	var PrimaryScreenRejectFlow=table.find('input[name=PrimaryScreenRejectFlow]').val();
	var BlendChestCy=table.find('input[name=BlendChestCy]').val();
	var Refiner1Power=table.find('input[name=Refiner1Power]').val();
	var Refiner2Power=table.find('input[name=Refiner2Power]').val();
	var Refiner1InletCy=table.find('input[name=Refiner1InletCy]').val();
	var Refiner2InletCy=table.find('input[name=Refiner2InletCy]').val();
	
	
	
	var qcsBasisWtTarget=table.find('input[name=qcsBasisWtTarget]').val();
	var moisturetarget=table.find('input[name=moisturetarget]').val();
	var yankeemapflow=table.find('input[name=yankeemapflow]').val();
	var machinechestpumpspeed=table.find('input[name=machinechestpumpspeed]').val();
	...
	$.ajax({
		...
		data:{
			...
			//New Fields
			SPRLoadingFront : SPRLoadingFront,
			SPRLoadingBack : SPRLoadingBack,
			PickUpRollVacuum : PickUpRollVacuum,
			UhleBoxVacuum : UhleBoxVacuum,
			SPRVacuum : SPRVacuum,
			PrimaryScreenRejectFlow : PrimaryScreenRejectFlow,
			BlendChestCy : BlendChestCy,
			Refiner1Power : Refiner1Power,
			Refiner2Power : Refiner2Power,
			Refiner1InletCy : Refiner1InletCy,
			Refiner2InletCy : Refiner2InletCy,
			//basisweighttarget : basisweighttarget,
			moisturetarget : moisturetarget,
			yankeemapflow : yankeemapflow,
			machinechestpumpspeed : machinechestpumpspeed
		},
		success:function(data){
			if(data.status){
				idEle.val(data.id);
...
function saveCenterlineGrade(){
	...
	var sprloadingfrontMinC=table.find('input[name=sprloadingfrontMinC]').val();
	var sprloadingfront=table.find('input[name=sprloadingfront]').val();
	var sprloadingfrontMaxC=table.find('input[name=sprloadingfrontMaxC]').val();
	
	var sprloadingbackMinC=table.find('input[name=sprloadingbackMinC]').val();
	var sprloadingback=table.find('input[name=sprloadingback]').val();
	var sprloadingbackMaxC=table.find('input[name=sprloadingbackMaxC]').val();
	
	var pickuprollvacuumMinC=table.find('input[name=pickuprollvacuumMinC]').val();
	var pickuprollvacuum=table.find('input[name=pickuprollvacuum]').val();
	var pickuprollvacuumMaxC=table.find('input[name=pickuprollvacuumMaxC]').val();
	
	var uhleboxvacuumMinC=table.find('input[name=uhleboxvacuumMinC]').val();
	var uhleboxvacuum=table.find('input[name=uhleboxvacuum]').val();
	var uhleboxvacuumMaxC=table.find('input[name=uhleboxvacuumMaxC]').val();
	
	var sprvacuumMinC=table.find('input[name=sprvacuumMinC]').val();
	var sprvacuum=table.find('input[name=sprvacuum]').val();
	var sprvacuumMaxC=table.find('input[name=sprvacuumMaxC]').val();
	
	var primaryscreenrejectflowMinC=table.find('input[name=primaryscreenrejectflowMinC]').val();
	var primaryscreenrejectflow=table.find('input[name=primaryscreenrejectflow]').val();
	var primaryscreenrejectflowMaxC=table.find('input[name=primaryscreenrejectflowMaxC]').val();
	
	var blendchestcyMinC=table.find('input[name=blendchestcyMinC]').val();
	var blendchestcy=table.find('input[name=blendchestcy]').val();
	var blendchestcyMaxC=table.find('input[name=blendchestcyMaxC]').val();
	
	var refiner1powerMinC=table.find('input[name=refiner1powerMinC]').val();
	var refiner1power=table.find('input[name=refiner1power]').val();
	var refiner1powerMaxC=table.find('input[name=refiner1powerMaxC]').val();
	
	var refiner2powerMinC=table.find('input[name=refiner2powerMinC]').val();
	var refiner2power=table.find('input[name=refiner2power]').val();
	var refiner2powerMaxC=table.find('input[name=refiner2powerMaxC]').val();
	
	var refiner1inletcyMinC=table.find('input[name=refiner1inletcyMinC]').val();
	var refiner1inletcy=table.find('input[name=refiner1inletcy]').val();
	var refiner1inletcyMaxC=table.find('input[name=refiner1inletcyMaxC]').val();
	
	var refiner2inletcyMinC=table.find('input[name=refiner2inletcyMinC]').val();
	var refiner2inletcy=table.find('input[name=refiner2inletcy]').val();
	var refiner2inletcyMaxC=table.find('input[name=refiner2inletcyMaxC]').val();
	
	//New Fields Added Here
	var basisweighttargetMinC=table.find('input[name=basisweighttargetMinC]').val();
	var basisweighttarget=table.find('input[name=basisweighttarget]').val();
	var basisweighttargetMaxC=table.find('input[name=basisweighttargetMaxC]').val();
	
	var moisturetargetMinC=table.find('input[name=moisturetargetMinC]').val();
	var moisturetarget=table.find('input[name=moisturetarget]').val();
	var moisturetargetMaxC=table.find('input[name=moisturetargetMaxC]').val();
	
	
	var yankeemapflowMinC=table.find('input[name=yankeemapflowMinC]').val();
	var yankeemapflow=table.find('input[name=yankeemapflow]').val();
	var yankeemapflowMaxC=table.find('input[name=yankeemapflowMaxC]').val();
	
	var machinechestpumpspeedMinC=table.find('input[name=machinechestpumpspeedMinC]').val();
	var machinechestpumpspeed=table.find('input[name=machinechestpumpspeed]').val();
	var machinechestpumpspeedMaxC=table.find('input[name=machinechestpumpspeedMaxC]').val();
	...
	$.ajax({
		...
		data : {
			...
			sprloadingfrontMinC : sprloadingfrontMinC,
			sprloadingfront : sprloadingfront,
			sprloadingfrontMaxC : sprloadingfrontMaxC,

			sprloadingbackMinC : sprloadingbackMinC,
			sprloadingback : sprloadingback,
			sprloadingbackMaxC : sprloadingbackMaxC,

			pickuprollvacuumMinC : pickuprollvacuumMinC,
			pickuprollvacuum : pickuprollvacuum,
			pickuprollvacuumMaxC : pickuprollvacuumMaxC,

			uhleboxvacuumMinC : uhleboxvacuumMinC,
			uhleboxvacuum : uhleboxvacuum,
			uhleboxvacuumMaxC : uhleboxvacuumMaxC,

			sprvacuumMinC : sprvacuumMinC,
			sprvacuum : sprvacuum,
			sprvacuumMaxC : sprvacuumMaxC,

			primaryscreenrejectflowMinC : primaryscreenrejectflowMinC,
			primaryscreenrejectflow : primaryscreenrejectflow,
			primaryscreenrejectflowMaxC : primaryscreenrejectflowMaxC,

			blendchestcyMinC : blendchestcyMinC,
			blendchestcy : blendchestcy,
			blendchestcyMaxC : blendchestcyMaxC,

			refiner1powerMinC : refiner1powerMinC,
			refiner1power : refiner1power,
			refiner1powerMaxC : refiner1powerMaxC,

			refiner2powerMinC : refiner2powerMinC,
			refiner2power : refiner2power,
			refiner2powerMaxC : refiner2powerMaxC,

			refiner1inletcyMinC : refiner1inletcyMinC,
			refiner1inletcy : refiner1inletcy,
			refiner1inletcyMaxC : refiner1inletcyMaxC,

			refiner2inletcyMinC : refiner2inletcyMinC,
			refiner2inletcy : refiner2inletcy,
			refiner2inletcyMaxC : refiner2inletcyMaxC,
			
			
			basisweighttargetMinC : basisweighttargetMinC,
			basisweighttarget : basisweighttarget,
			basisweighttargetMaxC : basisweighttargetMaxC,
			
			
			moisturetargetMinC : moisturetargetMinC,
			moisturetarget : moisturetarget,
			moisturetargetMaxC : moisturetargetMaxC,
			
			yankeemapflowMinC : yankeemapflowMinC,
			yankeemapflow : yankeemapflow,
			yankeemapflowMaxC : yankeemapflowMaxC,
			
			machinechestpumpspeedMinC : machinechestpumpspeedMinC,
			machinechestpumpspeed : machinechestpumpspeed,
			machinechestpumpspeedMaxC : machinechestpumpspeedMaxC,
