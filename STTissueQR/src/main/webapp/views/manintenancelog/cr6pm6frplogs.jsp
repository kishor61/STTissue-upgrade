...
function saveData(jq){
	...
	var target1='${target}';
	var prodtypeorgradecode='';
	if(target1==='ADMIN' || target1==='OPERATOR' || target1==='OPERATOR4'|| target1==='OPERATOR6' || target==='OPERATOR5'){
		prodtypeorgradecode=tr.find('input[name=prodtypeorgradecode]').val();
				...
				<div class="table-selector"  style="background-color:#c2eadf73 !important; border: 1px solid #7e00ff42;">
					...
					<table>
						<tr>
							...
							<td>
							<security:authorize access="hasAnyRole('ADMIN,OPERATOR,OPERATOR4,OPERATOR6,OPERATOR5')">
								<input type="hidden" name="shiftdate" value="${shiftdate}">
								<security:authorize access="hasAnyRole('ADMIN,OPERATOR,OPERATOR4,OPERATOR6,OPERATOR5')">
									<button id="loadDataBtn"class="btn btn-success btn-sm">Load</button>
								</security:authorize>
								&nbsp;
							</security:authorize>
								<!-- <button id="addRowBtn">Add New Row</button> -->
								&nbsp;
								<button id="deleterowbutton"class="btn btn-danger btn-sm">
									Delete
								</button>
							</td>
...
<c:if test="${showdata}">
<div style="padding: 2px; bottom: 0;left: 5px;right: 5px;top: 150px;">
		...
		<c:if test="${fn:length(moniterdatas) eq 0 }">
			<tr style="height: 70px;">
				...
				<td>
					...
					<select name="shift" id="shift" style="width: 100px; padding: 2px;" required="required" tabindex="2">
