...
$(function(){
		$('#editbutton').click(function(){			
			...
			var eDate=$('#yielddatatable tbody input[name=edate]').val();
			//alert("Start Date::"+gradeDate);
			//alert("End Date::"+eDate);
			if(gradeDate=='' || typeof gradeDate ==='undefined'){
				alert("Select The Date.");
				return false;
			}
			if(gradeDate!='-1'){
				location.href='${editYieldURL}/edit/'+encodeURIComponent(gradeDate)+'/'+encodeURIComponent(eDate);
			}
...
$(function(){
	$('#deletebutton').click(function(){
		var deleterow=$('input[name=rowCheckbox]:checked').val();
		...
		<div class="block3" style="overflow: auto;">
			<div class="pageContent">
				...
				<div class="page-title">
					<span class="label">FRP DAILY YIELD REPORT</span>
				</div>
			<div class="table-selector">
					...
					<table>
						<tr>
							...
							<!--This Form Is For Export The Report Starts From Here -->
							<form action="${yieldexportURL}" method="POST">
	...
	<c:if test="${viewpage}">
			<table class="table" id="yielddatatable">
						...
						<tbody id="yielddatainput">
							<c:if test="${fn:length(frpqualities) eq 0 }">
							<tr>
								<td><b style="color: red;">No Data Found For The Related Search...</b></td>
							</tr>
							</c:if>
						<c:if test="${fn:length(frpqualities) > 0 }">
...
<!-- 								<th></th> -->
								...
								<th style="width:80px;background-color: greenyellow;"colspan="7">Rejects Flow</th>
								<th style="width:80px;"></th>
								<th style="width:80px;background-color: greenyellow;"colspan="7">Rejects Consistencies</th>
								<th style="width:80px;background-color: greenyellow;"colspan="3">Ash</th>
						</tr>			
						<tr>
								...
								<th style="width:200px;">DWW flow<br>from Mill</th>
							...
							<c:forEach items="${frpqualities}" var="frpqualities">
							
							<tr>
								<fmt:formatDate value="${frpqualities.date}"  pattern="MM-dd-yyyy" var="date"/>
								<td>
									<input type="radio" name="rowCheckbox" value="${date}">
									<input type="hidden"name="edate" value="${enddate}">
								</td>
								<td name="date">
									${date}
								</td>								
<%-- 								<fmt:formatDate value="${frpqualities.date}" pattern="MM/dd/yyyy"/> --%>
								
								<td name="sampleno">${frpqualities.dwwflowrommill}</td>
								<td name="rfdetrasherrejcts">${frpqualities.rfdetrasherrejcts}</td>
								<td name="rfhdcleaner">${frpqualities.rfhdcleaner}</td>
								<td name="rftertiarycoarse">${frpqualities.rftertiarycoarse}</td>
								
								<td name="rftertiaryfine">${frpqualities.rftertiaryfine}</td>
								<td name="rffrwdcleaner">${frpqualities.rffrwdcleaner}</td>
								<td name="rfprimcell">${frpqualities.rfprimcell}</td>
								<td name="rfseccell">${frpqualities.rfseccell}</td>
								<td name="rffsdclarifier">${frpqualities.rffsdclarifier}</td>
								<td name="cyofdww">${frpqualities.cyofdww}</td>
								<td name="rchdcleaner">${frpqualities.rchdcleaner}</td>
								<td name="rctertiarycoarse">${frpqualities.rctertiarycoarse}</td>
								<td name="rctertiaryfine">${frpqualities.rctertiaryfine}</td>
								<td name="rcfrwdcleaner">${frpqualities.rcfrwdcleaner}</td>
								<td name="rcprimcell">${frpqualities.rcprimcell}</td>
								<td name="rcseccell">${frpqualities.rcseccell}</td>
								<td name="rcfsdclarifier">${frpqualities.rcfsdclarifier}</td>
								<td name="ashdww">${frpqualities.ashdww}</td>
								<td name="ashpupler">${frpqualities.ashpupler}</td>
								<td name="ashtertiarypress">${frpqualities.ashtertiarypress}</td>
<%-- 										<td>${frpqualities.ylswdcsfrpproduction}</td> --%>
							</tr>
