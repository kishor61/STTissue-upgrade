...
<div class="page-title">
	<span class="label" style="font-size: 12px;">Edit Follow Up</span>
</div>
<br>
...
<table>
	...
	<tr>	
		...
		<c:if test="${empty folloupObj.timeline}">
			<td>
				<input type="text" name="timeline" value='<fmt:formatDate value="${folloupObj.timeline}"  pattern="MM-dd-yyyy"/>' readonly="readonly" disabled="disabled" style="width: 100px;">
			</td>
	...
	<c:if test="${not empty folloupObj.recurrence}">
		<td><%-- <input type="text" name="recurrence" value="${folloupObj.recurrence}" readonly="readonly" style="width: 100px;"> --%>
			<select name="recurrence" style="width: 104px; padding: 2px;">
				<option value="">Select</option>
				<c:forEach items="${recurrences}" var="r">
					<c:choose>
					<c:when test="${r.key eq folloupObj.recurrence}">
		...
		<td colspan="3">
			<select name="tagColor" style="width: 100px; padding: 3px;">
				...
				<c:forEach items="${colors}" var="colorCode">
					<c:choose>
						<c:when test="${colorCode eq folloupObj.tagColor}">
	...
	<tr>
		...
		<br>
		<a href='<spring:url value="/oimnotes/followup/history/${folloupObj.id}"></spring:url>' target="_blank">View Change History</a>
		<br>
		</td>
	</tr>
	<tr>
