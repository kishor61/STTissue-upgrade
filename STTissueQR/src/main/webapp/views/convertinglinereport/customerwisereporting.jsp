...
$(function(){
	$('#editrowbutton').click(function()
		...
		if(val==""){
			...
			$('[id^='+val+']').css('display','');
			$('[id^='+val+']').css("background-color", "#CB842E");
		}
	});
});
</script>

<spring:url value="/convertingline/skuspecification/forecaste/saveorupdate" var="saveConvertingLineDataURL"/>
		...
		<div class="block3" style="overflow: auto;">
			<div class="pageContent">
				...
				<div class="page-title">
					<span class="label">Converting Line Report Customer Wise</span>
				</div>
				<div class="table-selector">
					<spring:url value="/convertinglinereport/customerwise/view/data" var="viewURL"/>
					...
					<table style="margin: auto;">
						<tr>
							...
							<td>Start Month: 
								 <select name="month" id="month" style="width: 100px; padding: 2px;" required="required">
							...
							<td>Start Year: 
								 <select name="syear" id="syear" style="width: 100px; padding: 2px;" required="required">
									<option value="">Select</option>
									<c:if test="${syear eq ''}">
									...
									<c:if test="${syear eq '2016'}">
									<option value="2016" selected="selected">2016</option>
									<option value="2017">2017</option>
									<option value="2018">2018</option>
									<option value="2019">2019</option>
									</c:if>
									<c:if test="${syear eq '2017'}">
									<option value="2016">2016</option>
									<option value="2017" selected="selected">2017</option>
									<option value="2018">2018</option>
									<option value="2019">2019</option>
									</c:if>
									<c:if test="${syear eq '2018'}">
									<option value="2016">2016</option>
									<option value="2017">2017</option>
									<option value="2018" selected="selected">2018</option>
									<option value="2019">2019</option>
									</c:if>
									<c:if test="${syear eq '2019'}">
									<option value="2016">2016</option>
									<option value="2017">2017</option>
									<option value="2018">2018</option>
							...
							<td>End Month: 
								 <select name="emonth" id="emonth" style="width: 100px; padding: 2px;" required="required">
									<option value="">Select</option>
									<c:if test="${emonth eq ''}">
									<option value="01">January</option>
									<option value="02">February</option>
							...
							<td>End Year: 
								 <select name="year" id="year" style="width: 100px; padding: 2px;" required="required">
									<option value="">Select</option>
									<c:if test="${year eq ''}">
									<option value="2016">2016</option>
									<option value="2017">2017</option>
							...
							<c:if test="${export}">
								 ...
								 <c:if test="${roshanPurpose}">
								 	<td><input type="button" id="editDataBtn" onclick="$('#editFrom').submit();" value="Edit Forecast"></td>
								 </c:if>
							</c:if>
							<td>
								<a href='<spring:url value="/convertinglinereport/searchforecastcustomerandskucodewise"/>' target="_blank">Search Forecast ( By Customer And SKU Code )</a>
							</td>
						</tr>
					</table>
					</form>
<form id="exportFrom" action='<spring:url value="/convertinglinereport/reportview/export"/>'  method="GET" style="display:none; " target="_blank">
	<input type="hidden" name="sdate" value="${sdate}">
	<input type="hidden" name="edate" value="${edate}">
	<input type="hidden" name="custname" value="${searchedcust}">
</form>
<form id="editFrom" action='<spring:url value="/convertingline/reportview/editforecast"/>'  method="GET" style="display:none; " target="_blank">
	<input type="hidden" name="sdate" value="${sdate}">
	<input type="hidden" name="edate" value="${edate}">
	<input type="hidden" name="custname" value="${searchedcust}">
</form>
				</div>
<c:if test="${fn:length(customerWiseData) eq 0 }">
<center>			
				<table>
				<tr>
					<td colspan="16" style="font-size: 21px; color: #cb842e;">There is no Data For Perticular Search And Month Range. </td>
					<!-- You
						Have Not Entred Data (Forecast Value/Shipped Data/Oredered PO) For Perticular Search
						And Month Range. -->
				</tr>
				</table>
</center>
			</c:if>
<c:if test="${fn:length(customerWiseData)>0}">
...
<table class="table" id="yielddatatable">
						...
						<tr>
								<th class="trobjrow" colspan="3" style="width:80px;">Customer Information</th>
								<th class="trobjrow" colspan="1" style="width:80px;">Data</th>
								<th class="trobjrow" colspan="14" style="width:80px;">Forecast</th>
							</tr>
						<tr>
								...
								<c:if test="${January}">
									<th style="width:80px;">January<br /><b style="color: red;">${JanuaryYear}</b></th>
								</c:if>
								<c:if test="${February}">
									<th style="width:80px;">February<br /><b style="color: red;">${FebruaryYear}</b></th>
								</c:if>
								<c:if test="${March}">
									<th style="width:80px;">March<br /><b style="color: red;">${MarchYear}</b></th>
								</c:if>
								<c:if test="${April}">
									<th style="width:80px;">April<br /><b style="color: red;">${AprilYear}</b></th>
								</c:if>
								<c:if test="${May}">
									<th style="width:80px;">May<br /><b style="color: red;">${MayYear}</b></th>
								</c:if>
								<c:if test="${June}">
									<th style="width:80px;">June<br /><b style="color: red;">${JuneYear}</b></th>
								</c:if>
								<c:if test="${July}">
									<th style="width:80px;">July<br /><b style="color: red;">${JulyYear}</b></th>
								</c:if>
								<c:if test="${August}">
									<th style="width:80px;">August<br /><b style="color: red;">${AugustYear}</b></th>
								</c:if>
								<c:if test="${September}">
									<th style="width:80px;">September<br /><b style="color: red;">${SeptemberYear}</b></th>
								</c:if>
								<c:if test="${October}">
									<th style="width:80px;">October<br /><b style="color: red;">${OctoberYear}</b></th>
								</c:if>
								<c:if test="${November}">
									<th style="width:80px;">November<br /><b style="color: red;">${NovemberYear}</b></th>
								</c:if>
								<c:if test="${December}">
									<th style="width:80px;">December<br /><b style="color: red;">${DecemberYear}</b></th>
								</c:if>	
							</tr>
						</thead>
						<tbody id="yielddatainput">
									...
									<%! int loopcount=0; %>
									<c:forEach items="${customerWiseData}" var="data">
											<!-- Section Starts For Edit Forecaste -->
											<tr style="display: none;" id="<%=loopcount%>">
											...
											<!-- Section Ends For Edit Forecaste -->
											
											<tr>
													<%-- <td><fmt:formatDate value="${data.date}" pattern="MM-dd-yyyy" var="dateH"/>${dateH}</td> --%>
													<%-- <td rowspan="4"> ${data.month}</td> --%>
														<td rowspan="5" style="background-color: rgba(255, 188, 0, 0.22);">
															<input type="radio" name="rowCheckbox" value="<%=loopcount%>">
															<b> ${data.customer}</b>
														</td>
											...
											<tr>
													<!-- <td  rowspan="2"></td> -->
													<!-- <td  style="background-color: #ffbc00;"><b> Order Quantity</b></td> -->
											...
											<c:if test="${fn:length(customerWiseData)>1}">
										...
										<!-- For Grand Total -->
											<tr>
													<%-- <td><fmt:formatDate value="${data.date}" pattern="MM-dd-yyyy" var="dateH"/>${dateH}</td> --%>
													<%-- <td rowspan="4"> ${data.month}</td> --%>
													<td rowspan="5" style="background-color: rgb(90, 158, 48);"><b></b></td>
