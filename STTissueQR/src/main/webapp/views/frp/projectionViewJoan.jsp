						...
						<tr>
							<td>
							 ...
							 <c:if test="${jonFlag}">
								  <td>
								 	<input type="hidden" name="oldtype" value="${oldtype}">
								 	<button id="viewDataBtnjon">Load Data</button>
								 	<button id="exportBtn">Export</button>
								 </td>
							 </c:if>
							 <td> &nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;
...
<spring:url value="/frpprojectionreportjon/view/data" var="viewDataURL"/>
<spring:url value="/frpprojectionreportjon/view/data/john" var="viewDataURLJohn"/>				
<script type="text/javascript">
	$(function(){
		
$('#viewDataBtnjon').click(function(){
			...
			let date1 = new Date(data.sdate).getTime();
			  let date2 = new Date(data.edate).getTime();

			  if (date1 > date2) {
				  alert("Please chose end date grater than Start Date ...");
				  return;
			  } 
			location.href='${viewDataURLJohn}?'+$.param(data);
		});		
	});
</script>
...
<c:if test="${showFlag }">
<spring:url value="/frpprojectionreportjon/export" var="exportURL"/>
...
<spring:url value="/frpprojectionJon/new" var="newFormulaURL"/>
