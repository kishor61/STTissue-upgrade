		...
		<div class="block3" style="overflow: auto;">
			<div class="pageContent">
				...
				<div class="page-title">
					<span class="label">FRP Projection</span>
				</div>
				<br>
					...
					<table style="margin: auto;">
						<tr>
							<td>
							 ...
							 <td>
							 	<select name="type" tabindex="1" style="width: 150px; padding: 2px;">
...
<spring:url value="/frpprojectionreport/view/data" var="viewDataURL"/>				
<script type="text/javascript">
	$(function(){
...
<c:if test="${showFlag }">
<spring:url value="/frpprojectionreport/export" var="exportURL"/>
...
<div style="position: fixed;overflow: auto;top: 160px;bottom: 0px;width: 98%;left: 11px;right: 5px;">	
	<table class="table" style="margin: auto; width: auto;">
		<thead>
			<tr>
				<th rowspan="3" style="width: 80px;"><div>Grade Code</div></th>
				<th rowspan="3" style="width: 150px;"><div>Grade</div></th>
				<th rowspan="3" style="width: 80px;"><div>${sdate}</div></th>
				<c:forEach begin="0" end="${projectionData.length}">
					<th style="width: 80px;"><div>Inbound</div></th>
				</c:forEach>
				<th rowspan="3" style="width: 80px;"><div>Total</div></th>
			</tr>
			<tr>
		...
		<tbody>
				...
				<c:forEach items="${projectionData.projectionDatas}" var="pdata">
					<tr>
						...
						<td style="width:150px;"><div>${pdata.grade}</div></td>
						<td style="width:80px;"><div><fmt:formatNumber value="${pdata.weight}" maxFractionDigits="2"/> </div></td>
				...
				<tr>
					...
					<td style="width:80px;"><div><fmt:formatNumber value="${totalWeight}" maxFractionDigits="2"/></div></td>
					<c:forEach begin="0" end="${projectionData.length}" varStatus="index">
						...
						<td style="width:80px;"><div><fmt:formatNumber value="${inboundTotal}" maxFractionDigits="2"/> </div></td>
					</c:forEach>
					<td style="width:80px;"><div><fmt:formatNumber value="${netTotal}" maxFractionDigits="2"/></div></td>
				</tr>
		
		
		
			<tr>
				<td colspan="${projectionData.length+5}" style="height: 30px;">
		...
		<%-- <c:if test="${type eq 'WH' }"> --%>
			...
			<tr>
				<td colspan="3">MWL And SWL (1/7)</td>
			...
			<%-- <tr>	
				<td colspan="3">SOW</td>
				<c:forEach items="${projectionData.sow}" var="sow">
					<c:if test="${sow lt 0 }">
						<td class="redcolor" style="width: 80px;"><div><fmt:formatNumber value="${sow}" maxFractionDigits="2" /> </div></td>
					</c:if>
					<c:if test="${sow ge 0 }">
						<td style="width: 80px;"><div><fmt:formatNumber value="${sow}" maxFractionDigits="2" /> </div></td>
					</c:if>
				</c:forEach>
			</tr> --%>
			<tr>
				<td colspan="3">SOW (13)</td>
			...
			<tr>	
				<td colspan="3">Groundwood(6/8/9)</td>
			...
			<tr>
				<td colspan="3">Mail-Undeliverable (20)</td>
			...
			<tr>	
				<td colspan="3">Others (30/60/65)</td>
				...
				<%-- <tr>	
				<td colspan="3">Others White</td>
				<c:forEach items="${projectionData.othersW}" var="othersW">
					<c:if test="${othersW lt 0 }">
						<td class="redcolor" style="width: 80px;"><div><fmt:formatNumber value="${othersW}" maxFractionDigits="2" /> </div></td>
					</c:if>
					<c:if test="${othersW ge 0 }">
						<td style="width: 80px;"><div><fmt:formatNumber value="${othersW}" maxFractionDigits="2" /> </div></td>
					</c:if>
				</c:forEach>
			</tr> --%>
			<tr>	
				...
				<c:forEach begin="0" end="${projectionData.length}" varStatus="index">
					...
					<select class="phyprop" name="projectionId" style="width: 75px;padding: 2px;">
			...
			<tr>	
				<td colspan="3"><b>Total avail</b></td>
				<c:forEach begin="0" end="${projectionData.length}" varStatus="i">
					...
					<%-- <c:set var="availTotal" value="${availTotal+projectionData.sow[i.index]}" /> --%>
					...
					<%-- <c:set var="availTotal" value="${availTotal+projectionData.othersW[i.index]}" /> --%>
