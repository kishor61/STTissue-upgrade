...
<spring:url	
	value="/qulitycheck_pm5/qualitychecklist/check" var="checkURL" />
<spring:url
	value="/qulitycheck_pm5/qualitychecklist/save" var="saveURL" />
<spring:url
	value="/qulitycheck_pm5/productspecificationsignoffsheet/delete"
	var="deletewinderDataURL" />
<spring:url
	value="/qulitycheck_pm5/productspecificationsignoffsheet/view" var="oldDataURL" />
	<spring:url value="/qulitycheck_pm5/timediff/save" var="timediffURL"/>
<style>
.inpCls {
	width: 90px;
}
							...
							if(data.count<8)
							{
								if(data.check)
								{
								 ...
								 location.reload(true);
									...
									alert('Machine already down in given start and End time please chose another time ');
								}
							}else{
...
var clearTimer;
$(function() {
	$('#yielddatatable tbody input, #yielddatatable tbody select').unbind('focusin focusout');
	$('#yielddatatable tbody input, #yielddatatable tbody select').on('focusin', doFocusIn).on('focusout', doFocusOut);
		...
		$.ajax({
			...
			success: function(data) {
				if(data.count < 8) {
					...
					var newRow = $('#yielddatatable tbody tr:first').clone();
					...
					// Set the date value
					newRow.find('input[name="sdate"]').val(sdate);
					
					// Add the new row to the table
					$('#yielddatatable tbody').append(newRow);
					
					// Set up event handlers for the new row
					newRow.find('input').unbind('focusin focusout');
					newRow.find('select').unbind('focusin focusout');
					newRow.find('input').on('focusin', doFocusIn).on('focusout', doFocusOut);
					newRow.find('select').on('focusin', doFocusIn).on('focusout', doFocusOut);
				} else {
					alert('Maximum number of entries (8) reached for today');
				}
			},
			error: function(xhr, status, error) {
				alert('Server error occurred while adding new row');
			}
		});
	});
});
				...
				<div class="filter-section">
					<div class="controls-row">
						<div class="control-group">
							<label>Date:</label>
							<input readonly="readonly" type="text" name="sdate" id="sdate" value="${currentdate}" style="width: 100px;">
						</div>
						<div class="control-group">
							...
							<button id="deleterowbutton" class="btn btn-danger">Delete</button>
						</div>
						<div class="control-group">
							<label style="font-weight:bold;font-size: large">Machine DownTime</label>
							<div class="time-selectors">
								...
								<button id="machinedown" class="btn btn-warning">Submit</button>
							</div>
						</div>
					</div>
					<div class="filter-row">
						<span class="blink_me" id="timeDiff">Total Duration: ${totaldiff}Hr</span>
					</div>
				</div>
					...
					<table id="yielddatatable" class="table">
						<thead>
							<tr>
								...
								<th>Position</th>
								<th>Width of Roll</th>
								<th>Diameter of Roll</th>
								<th>Core</th>
								<th>Corrugation</th>
								<th>Holes</th>
								<th>EdgeQuality</th>
								<th>R1/R2 Initial</th>
								<th>Auditer Initial</th>
							</tr>
						</thead>
						<tbody>
								...
								<c:forEach items="${data}" var="datas">
									<c:if test="${datas.machinedown=='yes' }">
										<tr>
											...
											<td colspan="14" style="background-color: red;text-align: center;font-size: 15px">
												Machine Down &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
												<fmt:formatDate value="${datas.stime}" var="dateS" pattern="HH:mm" />  
												Start Time : ${dateS} 
												<fmt:formatDate value="${datas.etime}" var="dateE" pattern="HH:mm" />  
												End Time : ${dateE}  
											</td>                                    
										</tr>                
									</c:if>
									<c:if test="${datas.machinedown=='No' }">
										<tr>
											...
											<td>
												<select name="gradecode" id="gradecode" style="width: 98px; padding: 2px;" required="required">
