...
<spring:url value="/frpefficiency/timedurtation" var="timedurtationURL" />

<spring:url value="/frpefficiency/timediff" var="timediffURL" />
<spring:url value="/frpefficiency/savenew" var="saveURL" />
<spring:url value="/frpefficiency/delete" var="deleteURL" />
<spring:url value="/frpefficiency/current/info" var="currentInfoURL" />
...
<script type="text/javascript">
	...
	$(function() {
		...
		$('select[name=reel]').focusin(setCurrentValue);
 ...
 */
		...
		$('select[name=reel]').focusout(reel);
		...
		$('#addNewBtn').click(
				function() {
					...
					ntr.find('select[name=reel]').val(otr.find('select[name=reel]').val());
					...
					ntr.find('select[name=reel]').focusout(reel);
					...
					ntr.find('select[name=reel]').focusout(setCurrentValue);
...
function fendTimeHH() {
	...
	function saveData(jq) {
		...
		if (jq.val() != currentValue) {
			...
			var reel=tr.find('select[name=reel]').val();
...
<div class="content-wrapper" style=" padding-top: 0px !important; background-color: #d4d8e559 !important;">
	...
	<div class="block3" style="overflow: auto;">
				<div class="page-title">
					<span class="label">FRP Efficiency- Data Entry</span>
				</div>
				<br>
<security:authorize access="hasAnyRole('OPERATOR2')">
<spring:url value="/frpefficiency/new" var="oldDataURL" />
<script type="text/javascript">
	$(function(){
		$('input[name=edate]').datepicker({
			dateFormat : 'mm-dd-yy',
	...
	$(function(){
		...
		$('#loadDataBtn').click(function(){
			...
			if(shiftDate!=odate){
				...
				<div class="table-selector">
					...
					<table>
						<tr>
							...
							<td><security:authorize access="hasAnyRole('ADMIN,OPERATOR2')">
				...
				<div
					style="position: absolute;   overflow: auto;width: 98%;bottom: 0;top: 102px;">
...
<%-- <td style="width: 70px;">
	...
	<td style="width: 80px;">
	<%-- 	<input type="text" name="reel" value="${ef.batchNo}"  autocomplete="off"> --%>
	<select name="reel">
			<option value="">Select</option>
			<c:forEach items="${batchNos}" var="batchNo">
				<c:choose>
					<c:when test="${batchNo.key eq ef.batchNo}">
						<option value="${batchNo.key}" selected="selected">${batchNo.value}</option>
					</c:when>
					<c:otherwise>
						<option value="${batchNo.key}">${batchNo.value}</option>
					</c:otherwise>
				</c:choose>
			</c:forEach>
		</select>	
	...
	<td style="width: 100px;">
		<select name="gradeCode">
			...
			<c:forEach items="${grades}" var="grade">
				<c:choose>
					<c:when test="${grade.key eq ef.grade}">
