 ...
 */
@Component
public class NewDespatchReportHandler {
	...
	public void createExcelDespatchReport(List<Despatch> datas,String tblName, FileOutputStream outputStream) throws IOException {
		...
		workbookUtil.addValue(row, col++, "Freight amount", Workbook2007Util.Style.STYLE_BOLD_CENTER, rowHeight);
		...
		for (Despatch data : datas) {
			...
			int checkCarrierRate=(int) despatchService.getCarrirRateTempTable(tblName,data.getReleasenumber(),data.getCarrierRate());
			if(checkCarrierRate==0){
				workbookUtil.addValue(row, col++, data.getCarrierRate(), Workbook2007Util.Style.STYLE_NORMAL_CENTER_Yellow, rowHeight);
			}else{
				workbookUtil.addValue(row, col++, data.getCarrierRate(), Workbook2007Util.Style.STYLE_NORMAL_CENTER, rowHeight);
			}
	...
	private void insertDataInTempTable(List<Despatch> datas) {
		...
		for (final Despatch data : datas) {
			...
			final String sql = "insert into [newtbldispatch_temp]"
					+ "("
					+ "[PickUpDate],"
					+ "[PickUpPoint],"
					+ "[PickUpCity],"
			...
			jdbcTemplate.update(new PreparedStatementCreator() {
				...
				public PreparedStatement createPreparedStatement(Connection arg0)
					...
					statement.setInt(7, data.getCarrierRate());
	...
	private void truncateTable() {
		...
		try {
			JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
			String sql = "Truncate table [newtbldispatch_temp]";
