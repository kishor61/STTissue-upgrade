...
import com.st.efficiencypm5.automailer.EfficiencyAutoMailerPM5;
...
import com.st.efficiencypm5.report.EfficiencyReportByDCSHandlerPM5;
import com.st.efficiencypm5.report.EfficiencyReportHandlerPM5;
 ...
 */
@Controller
@RequestMapping(value="/pm5efficiencyreport")
public class EfficiencyReportControllerPM5 {
	...
	@Autowired
	private EfficiencyReportHandlerPM5 reportHandler;
	
	@Autowired
	private EfficiencyReportByDCSHandlerPM5 efficiencyreportbydcshandler; 
	
	@Autowired
	private EfficiencyAutoMailerPM5 efficiencyAutoMailer;
	...
	public String view(HttpServletRequest request,
		...
		if(sdate==null){
			...
			return "redirect:/pm5efficiencyreport/main";
		}
		...
		int hh=0;
		int mm=0;
		for (EfficiencyPM5 efficiency2 : efficiencies) {
	...
	public void export(HttpServletRequest request,
		...
		File file=new File(context.getRealPath("/")+"WEB-INF/Efficiency Report_PM5.xls");
	 ...
	 * @return
	 */
	private List<Map<String, Object>> formatData(List<EfficiencyPM5> efficiencies) {
		...
		for (EfficiencyPM5 efficiency : efficiencies) {
			...
			SecondaryCodePM5 secondaryCode=efficiency.getSecondaryCode();
			PrimaryCodePM5 primaryCode=null;
	...
	//	model.addAttribute("scodes", efficiencyCodeService.getSecondaryCodes());
		...
		return "PM5/efficiencyReportSummary";
	}
	...
	public String viewSummary(HttpServletRequest request,
		...
		if(sdate==null){
			redirectAttributes.addFlashAttribute("error", "Invalid start date");
			return "redirect:/pm5efficiencyreport/summary";
		}
	...
	private HSSFWorkbook getFormatedSummaryWorkbook(File file,
		...
		List<EffSummaryPrimaryPM5> effSummaryPrimaries=(List<EffSummaryPrimaryPM5>) datas.get("summaryPrimaries");
	...
	*/
	...
	//	model.addAttribute("shift", "ALL");
		...
		return "PM5/efficiencyReportSummaryByShift";
	}
	...
	public String efficiencyReportByShiftData(@RequestParam("sdate")String sdate,
		...
		List<EfficiencyShiftDataPM5> datas=null;
		List<EfficiencyShiftDataPM5> datas1=null;
		...
		for (EfficiencyShiftDataPM5 data : datas1) {
		...
		for (String temp : list1) {
			...
			if(temp.equals(dateFormat.format(currentDate))){
				...
				if(Collections.frequency(list1, temp)==1){
					...
					datas = new ArrayList<EfficiencyShiftDataPM5>();
					EfficiencyShiftDataPM5 cash = new EfficiencyShiftDataPM5();
		...
		List<EfficiencyShiftDataPM5> newList = new ArrayList<EfficiencyShiftDataPM5>();
		if(datas==null){
		...
		Collections.sort(newList, new Comparator<EfficiencyShiftDataPM5>() {
			  public int compare(EfficiencyShiftDataPM5 o1, EfficiencyShiftDataPM5 o2) {
	...
	//	model.addAttribute("shift", shift);
		...
		model.addAttribute("aData", new EfficiencyShiftDataPM5().totalA(newList));
		model.addAttribute("bData", new EfficiencyShiftDataPM5().totalB(newList));
		model.addAttribute("cData", new EfficiencyShiftDataPM5().totalC(newList));
		model.addAttribute("dData", new EfficiencyShiftDataPM5().totalD(newList));
		model.addAttribute("totalData", new EfficiencyShiftDataPM5().total(newList));
	...
	public String efficiencyReportByDCSView(Model model){
		...
		return "PM5/efficiencyReportSummaryByDCS";
	}
	...
	public String efficiencyReportByDCSData(HttpServletRequest request,HttpServletResponse response,Model model,RedirectAttributes redirectAttributes){
		...
		final EfficiencyShiftDataPM5 eSD= new EfficiencyShiftDataPM5();
		...
		List<EfficiencyShiftDataPM5> checkTon=efficiencyService.checkTonPresentOrNot(sdate,shift);
		if(checkTon.size()>0){
			...
			return "redirect:/pm5efficiencyreport/byDcs";
		}else{
			eSD.setDate(sdate);
			eSD.setShift(shift);
	...
	public String efficiencyReportByDCSReport(Model model){
		...
		return "PM5/efficiencyReportSummaryByDCSReport";
	}
	...
	public String efficiencyReportByDCSReportView(@RequestParam("sdate")String sdate,
		...
		List<EfficiencyShiftDataPM5> datas=efficiencyService.getEfficiencyShiftData(CommonUtil.checkDate(sdate, dateFormat),CommonUtil.checkDate(edate, dateFormat),"");
		List<EfficiencyShiftDataPM5> datas1=efficiencyService.getEfficiencyDCSDataReport(CommonUtil.checkDate(sdate, dateFormat),CommonUtil.checkDate(edate, dateFormat));
		
		SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yyyy");
		for(EfficiencyShiftDataPM5 outdata : datas){
			//Date date1=(Date)sdf.parse(""+outdata.getDate());
			Date date1 = new Date();
			date1=outdata.getDate();
			//System.out.println("Date1::"+date1);
			String shift1=outdata.getShift();
			for(EfficiencyShiftDataPM5 innerdata : datas1){
		...
		model.addAttribute("edate", edate);
		model.addAttribute("showFlag", true);
		model.addAttribute("aData", new EfficiencyShiftDataPM5().totalA(datas));
		model.addAttribute("bData", new EfficiencyShiftDataPM5().totalB(datas));
		model.addAttribute("cData", new EfficiencyShiftDataPM5().totalC(datas));
		model.addAttribute("dData", new EfficiencyShiftDataPM5().totalD(datas));
		model.addAttribute("totalData", new EfficiencyShiftDataPM5().total(datas));
	...
	List<EfficiencyShiftDataPM5> reportDataEdit=efficiencyService.editDCSReport(id);
