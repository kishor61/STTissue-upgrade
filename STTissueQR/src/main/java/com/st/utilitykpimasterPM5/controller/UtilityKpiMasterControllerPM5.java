/**
 *Feb 1, 2018
 *UtilityKpiMasterControllerPM5.java
 * TODO
 *com.st.utilitykpimasterPM5.controller
 *UtilityKpiMasterControllerPM5.java
 *Roshan Lal Tailor
 */
package com.st.utilitykpimasterPM5.controller;
...
import com.st.utilitykpimasterPM5.service.MasterDataServicePM5;
import com.st.utilitykpimasterPM5.service.UtilityDataServicePM5;
 ...
 */
@Controller
@RequestMapping(value="/pm5utilitykpimaster")
public class UtilityKpiMasterControllerPM5 {
	
	@Autowired
	private UtilityDataServicePM5 utilityDataServicePM5;
	
	@Autowired
	private MasterDataServicePM5 masterDataServicePM5;
	...
	@Autowired
	private KpiRecordableDatePM5Service kpiRecordableDatePM5Service;
	...
	public String main(Model model) {
		...
		model.addAttribute("pages", UtilityKpiMasterCommon.getEntryPagesPM5());
		return "PM5/utilityKpiMasterMain";
	...
	public String entryPage(
		...
		if(page.equalsIgnoreCase("U")){
			List<UtilityData> utilityDatas=utilityDataServicePM5.getUtilityDatasL31();
			model.addAttribute("utilityDatas", utilityDatas);
		}else if(page.equalsIgnoreCase("M")){
			
			List<Map<String, Object>> centerlineGrades=centerlineServicePM5.getCenterlineGrades();
			model.addAttribute("centerlineGrades", UtilityKpiMasterCommon.getJsonDataOfCenterlineGrade(centerlineGrades));
			
			List<MasterData> masterDatas=masterDataServicePM5.getMasterDatasL31();
			model.addAttribute("masterDatas", masterDatas);
			
		}else if(page.equalsIgnoreCase("K")){
			List<KpiRecordableDate> kpiRecordableDates=kpiRecordableDatePM5Service.getKpiRecordableDate();
	...
	public String edit(
		...
		if(page.equalsIgnoreCase("U")){
			...
			utilityDatas.add(utilityDataServicePM5.getUtilityData(id));
			model.addAttribute("utilityDatas", utilityDatas);
		}else if(page.equalsIgnoreCase("M")){
			List<Map<String, Object>> centerlineGrades=centerlineServicePM5.getCenterlineGrades();
			model.addAttribute("centerlineGrades", UtilityKpiMasterCommon.getJsonDataOfCenterlineGrade(centerlineGrades));
			...
			masterDatas.add(masterDataServicePM5.getMasterData(id));
			model.addAttribute("masterDatas", masterDatas);
		}
		
		
		
		return "PM5/utilityKpiMasterEdit";
		...
		try {
			...
			try {
				if(utilityData.getId()==0){
					int key=utilityDataServicePM5.save(utilityData);
					
					UtilityData prevUtilityData=utilityDataServicePM5.getPrevUtilityData(key);
					...
					utilityDataServicePM5.update(utilityData);
					
					UtilityData prevUtilityData=utilityDataServicePM5.getPrevUtilityData(id);
	...
	public void delete(HttpServletRequest request,HttpServletResponse response) throws IOException {
		...
		try {
			utilityDataServicePM5.delete(id);
			map.put("status", true);
		} catch (Exception e) {
			map.put("status", false);
		...
		try {
			...
			try {
				if(masterData.getId()==0){
					int key=masterDataServicePM5.save(masterData);
					map.put("status", true);
					map.put("id", key);
					map.put("message", "Data saved successfully.");
				}else{
					masterDataServicePM5.update(masterData);
					map.put("status", true);
	...
	public void deleteMaster(HttpServletRequest request,HttpServletResponse response) throws IOException {
		...
		try {
			masterDataServicePM5.delete(id);
			map.put("status", true);
		} catch (Exception e) {
			map.put("status", false);
			map.put("error", "Server error...");
	...
	public void saveKpiRecordableDate(HttpServletRequest request,HttpServletResponse response) throws IOException {
		...
		try {
			...
			try {
				...
				int key=kpiRecordableDatePM5Service.save(kpiRecordableDate);				
//				System.out.println("Id="+key);
	...
	public void deleteKpiRecordableDate(HttpServletRequest request,HttpServletResponse response) throws IOException {
		...
		try {
			kpiRecordableDatePM5Service.delete(id);
			map.put("status", true);
		} catch (Exception e) {
			map.put("status", false);
			map.put("error", "Server error...");
	...
	public String shbackdatedentry(Model model){
		...
		return "PM5/utilityKpiMasterBackDatedEntryMain";
	}
	...
	List<MasterData> masterDatas=masterDataServicePM5.getMasterDatasBetweenDates(Sdate,Edate);
	model.addAttribute("masterDatas", masterDatas);
	model.addAttribute("page", "M");
