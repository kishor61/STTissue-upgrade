...
import com.st.production.model.WrapProductionRedCode.WrapperData;
import com.st.production.model.WrapperProduction;
import com.st.production.util.WrapperRedCodeComparator;
import com.st.production.util.WrapperRedCodeComparator.Field;
 ...
 */
@Service
public class WrapperProductionServiceImp implements WrapperProductionService {
	...
	public List<Map<String, String>> formatDataForWrappedReport(Date sdate,
		...
		for(int i=0;i<days+1;i++){
			...
			*/
			...
			List<WrapperProduction> wrapperProductions=wrapperProductionDao.getWrapperProductionsByMonth(scal.getTime(),ecal.getTime());
...
//		ecal.add(Calendar.MONTH, 1);
		...
		while(!(scal.get(Calendar.MONTH)==ecal.get(Calendar.MONTH)
			...
			*/	
				...
				List<MachineProduction> machineProductions=machineProductionDao.getMachineProductions(scalm.getTime(),ecalm.getTime());
				for (MachineProduction machineProduction : machineProductions) {
					machineProductionTotal+=machineProduction.getGoodWeight();
				}
				...
				List<WrapperProduction> wrapperProductions=wrapperProductionDao.getWrapperProductionsByMonth(scalm.getTime(),ecalm.getTime());
				for (WrapperProduction wrapperProduction : wrapperProductions) {
					String gradeCode=wrapperProduction.getGradeCode();
					if(StringUtils.isEmpty(gradeCode)){
						continue;
	...
	public Map<String, Object> getWrapperDataByRedCode(Date sdate, Date edate) throws ProductionException {
		...
		List<WrapProductionRedCode> redCodes=wrapperProductionDao.getRedCodes();
		...
		if(month>=0){
			...
			for(int i=0;i<=month;i++){
				...
				List<WrapperProduction> productions=wrapperProductionDao.getWrapperProductionRedCode(scal.getTime(),ecal.getTime());
				
				for(WrapProductionRedCode redCode:redCodes){
					
					List<WrapperProduction> wrapperProductions=getWrapperProduction(redCode.getRedCode(),productions);
					...
					if(wrapperProductions.size()>0){
						...
						for (WrapperProduction wrapper : wrapperProductions) {
		...
		//Filter
		List<WrapProductionRedCode> redCodesData= new ArrayList<>();
		
		for (int i=0;i<redCodes.size();i++) {
			WrapProductionRedCode wrapProductionRedCode=redCodes.get(i);
		...
		WrapperRedCodeComparator comparator=new WrapperRedCodeComparator();
		...
		WrapProductionRedCode redCodeNone=new WrapProductionRedCode();
		...
		if(month>=0){
			...
			for(int i=0;i<=month;i++){
				...
				List<WrapperProduction> productions=wrapperProductionDao.getWrapperProductionNoneRedCode(scal.getTime(),ecal.getTime());
				
			//	for(WrapProductionRedCode redCode:redCodes){
					
					List<WrapperProduction> wrapperProductions=getWrapperProduction(redCodeNone.getRedCode(),productions);
		...
		{
			WrapProductionRedCode totalData=new WrapProductionRedCode();
			List<WrapperData> totalWrapperDatas=new ArrayList<WrapProductionRedCode.WrapperData>();
			...
			for (int i = 0; i < months.size(); i++) {
				...
				for (WrapProductionRedCode wrapProductionRedCode : redCodesData) {
	 ...
	 * @return
	 */
	private List<WrapperProduction> getWrapperProduction(String redCode,
			List<WrapperProduction> productions) {
		List<WrapperProduction> productionsList=new ArrayList<>();
	...
	public HSSFWorkbook getFormatedWorkbookForWrapperRedCode(File file,
		...
		List<WrapProductionRedCode> redCodesData=(List<WrapProductionRedCode>)data.get("redData");
		...
		{
			WrapProductionRedCode wrapProductionRedCode=(WrapProductionRedCode)data.get("totalRedData");
			cellCount=0;
	...
	@Transactional
	@Override
	public List<WrapperProduction> getInventoryWrapperProductions(Date date,
			String customer) {
	...
	@Transactional
	@Override
	public List<WrapperProduction> getWrapperDataByDate(Date sdate,
			Date edate) throws ProductionException {
	...
	@Override
	@Transactional
	public List<WrapperProduction> getWrapperRedRejectProductionData_PM5(Date sdate, Date edate) {
		// TODO Auto-generated method stub
