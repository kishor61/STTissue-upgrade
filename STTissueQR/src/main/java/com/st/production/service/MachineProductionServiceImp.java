 ...
 */
@Service
public class MachineProductionServiceImp implements MachineProductionService {
	...
	@Transactional
	@Override
	public List<MachineProduction> getMachineProductions(Date sdate, Date edate) throws ProductionException {
		...
		//	System.out.println("End Date:-"+ecal.getTime());
			...
			if(oldDownTime==0){
				...
				for (Efficiency eff : efficiencies) {
					PrimaryCode primaryCode=eff.getSecondaryCode().getPrimaryCode();
	...
	public Map<String, Object> formatDataForSumaryReport(Date sdate,
		...
		List<MachineProductionSummary> productionSummaries=new ArrayList<>();
	
		
		//Code Starts From Here Done By Roshan Tailor Date:-2015-11-23
		
		List<MachineProduction> checkMachineProductionsForRollBreakData=machineProductionDao.checkMachineProductionsForRollBreak(scal.getTime(), ecal.getTime(),shift);
		
		List<MachineProduction> machineProductionsForRollBreak=machineProductionDao.getMachineProductionsForRollBreak(scal.getTime(), ecal.getTime(),shift,checkMachineProductionsForRollBreakData.size());
		datas.put("machinerollbreakinfo", machineProductionsForRollBreak);
		//Code Ends Here Done By Roshan Tailor Date:-2015-11-23
				
		List<MachineProduction> machineProductions=machineProductionDao.getMachineProductions(scal.getTime(), ecal.getTime(),shift);
		for (MachineProduction machineProduction : machineProductions) {
			MachineProductionSummary productionSummary=getMachineProductionSummary(productionSummaries,machineProduction.getGradeCode());
			
			
			
			if(productionSummary==null){
				productionSummary=new MachineProductionSummary();
		...
		WrapperProductionSummary dayWrapperProduction=new WrapperProductionSummary();
		dayWrapperProduction.setShift("Day");
		WrapperProductionSummary nightWrapperProduction=new WrapperProductionSummary();
		...
		List<WrapperProduction> checkBreaksData=wrapperProductionDao.checkBreaks(scal.getTime(), ecal.getTime(),shift);
		
		System.out.println(checkBreaksData.size());
		List<WrapperProduction> wrapperProductionsForRollBreak=null;
		...
		for(WrapperProduction data :wrapperProductionsForRollBreak){
		...
		List<WrapperProduction> wrapperProductions=wrapperProductionDao.getWrapperProductions(scal.getTime(), ecal.getTime(),shift);
		for (WrapperProduction wrapperProduction : wrapperProductions) {
		...
		scalMTD.set(Calendar.HOUR_OF_DAY, 7);
		...
		MachineProductionSummary mpMTD=new MachineProductionSummary();
		double dayMpDay=0;
		double nightMpDay=0;
		double mpReject=0;
		List<MachineProduction> machineProductionsMTD=machineProductionDao.getMachineProductions(scalMTD.getTime(), ecal.getTime(),"");
		for (MachineProduction machineProduction : machineProductionsMTD) {
		...
		WrapperProductionSummary wpMTD=new WrapperProductionSummary();
		List<WrapperProduction> wrapperProductionsMTD=wrapperProductionDao.getWrapperProductions(scalMTD.getTime(), ecal.getTime(),"");
		double wrWhite=0;
		double wrRed=0;
		double wrReject=0;
		for (WrapperProduction wrapperProduction : wrapperProductionsMTD) {
	 ...
	 * @return
	 */
	private MachineProductionSummary getMachineProductionSummary(
			List<MachineProductionSummary> productionSummaries, String gradeCode) {
		MachineProductionSummary productionSummary=null;
	...
	public HSSFWorkbook getFormatedWorkbookForSummaryReport(File file,Map<String, Object> datas,Date date) throws IOException {
		...
		List<MachineProductionSummary> productionSummaries=(List<MachineProductionSummary>)datas.get("production");
		MachineProductionSummary mpMTD=(MachineProductionSummary) datas.get("mpMTD");
		
		
		
		
		WrapperProductionSummary dayWrap=(WrapperProductionSummary) datas.get("wrapDay");
		WrapperProductionSummary nightWrap=(WrapperProductionSummary) datas.get("wrapNigh");
	...
	@Transactional
	@Override
	public List<MachineAndWrapper> getMachineAndWrapper(Date sdate, Date edate) throws ProductionException {
