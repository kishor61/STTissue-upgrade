...
import com.st.production.model.BillofLadingHeaderDetail;
 ...
 */
@Repository
public class WrapperProductionDaoImp implements WrapperProductionDao {
	
	private static final Logger logger=LoggerFactory.getLogger(WrapperProductionDaoImp.class);
	...
	@Override
	public List<WrapperProduction> getWrapperProductions(Date sdate, Date edate) throws ProductionException {
		...
		+ " (g.[GradeCode]=w.[GradeCode]) "
		+ " And "
		+ " w.[WrapperNumber] <> 'ZZZ' AND w.[WrapperNumber]='WR6' "
		+ " order by w.[DateTimeEntered]";
	//	System.out.println(sql);
		
		List<WrapperProduction> wrapperProductions=null;
		try {
			wrapperProductions=jdbcTemplate.query(sql,
					...
					new RowMapper<WrapperProduction>(){

						@Override
						public WrapperProduction mapRow(ResultSet rs, int arg1)
								throws SQLException {
							
							WrapperProduction wrapperProduction=new WrapperProduction();
	...
	public double getTrageTons(List<String> gradeCodes) throws ProductionException {
		...
		Double targetTons=Double.valueOf(0);
		...
		if(gradeCodes.size()>0){
			...
			try {
				targetTons=jdbcTemplate.queryForObject(sql, Double.class);
	 ...
	 */
	@Override
	public List<WrapperProduction> getWrapperProductionsByMonth(Date sdate,
	 ...
	 */
	@Override
	public List<WrapperProduction> getWrapperProductions(Date sdate,
	...
	String sql="select "
			...
			+ " and "
			+ "[WrapperNumber] <> 'ZZZ'  AND [WrapperNumber]='WR6' ";
	...
	@Override
	public List<WrapperProduction> getWrapperProductionRedCode(Date sdate,
			Date edate) throws ProductionException {
	...
	@Override
	public List<WrapProductionRedCode> getRedCodes() throws ProductionException {
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSourceP);
		
		String sql="SELECT [RedCode], [RedCodeDescription] "
				+ " FROM tlkpRedCode";

		List<WrapProductionRedCode> wrapperProductions=null;
			try {
				wrapperProductions=jdbcTemplate.query(sql,
						new RowMapper<WrapProductionRedCode>(){

							@Override
							public WrapProductionRedCode mapRow(ResultSet rs, int arg1)
									throws SQLException {
								
								WrapProductionRedCode redCode=new WrapProductionRedCode();
	 ...
	 */
	@Override
	public List<WrapperProduction> getInventoryWrapperProductions(Date date,String customer) {
		...
		List<WrapperProduction> productions=jdbcTemplate.query(sql, new Object[]{
			...
			public WrapperProduction mapRow(ResultSet rs, int arg1)
					throws SQLException {
				WrapperProduction production=new WrapperProduction();
			...
			//	System.out.println(sql2);
				...
				List<BillofLadingHeaderDetail> details=jdbcTemplate.query(sql2, new Object[]{
						production.getCustomerGradeCode(),
						production.getGradeCode()
					}, new RowMapper<BillofLadingHeaderDetail>(){

						@Override
						public BillofLadingHeaderDetail mapRow(ResultSet rs,
								int arg1) throws SQLException {
							BillofLadingHeaderDetail detail=new BillofLadingHeaderDetail();
				...
				double rollWeight=0;
				int rollCount=0;
				for (BillofLadingHeaderDetail billofLadingHeaderDetail : details) {
	 ...
	 */
	@Override
	public List<WrapperProduction> getWrapperProductionNoneRedCode(Date sdate,
			Date edate) throws ProductionException {
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSourceP);
		Object[] param=new Object[]{
						new Timestamp(sdate.getTime()),
	...
	public  List<Map<String, Object>> getGradeAndHoursData(Date sdate,Date edate) throws ProductionException {
		...
		//	System.out.println(ecal.getTime());
			...
			try {
				...
				List<MachineProduction> machineProductions=jdbcTemplate.query(sql,source, new RowMapper<MachineProduction>() {

					@Override
					public MachineProduction mapRow(ResultSet rs, int arg1) throws SQLException {
						MachineProduction machineProduction=new MachineProduction();
			...
			//		System.out.println(sql);
				...
				//	System.out.println();
					...
					List<WrapperProduction> wrapperProductions=jdbcTemplate.query(sql,source, 
							new RowMapper<WrapperProduction>(){
	 ...
	 */
	@Override
	public List<WrapperProduction> getWrapperForDailyInventory(Date sdate,
			Date edate) {
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSourceP);
		
		List<WrapperProduction> productions=new ArrayList<WrapperProduction>();
		...
		for (int i = 0; i < days+1; i++) {
			...
			//White Weight
			{
				String sql=" SELECT "
						...
						+ " [DateTimeEntered] between ? and ? "
						+ " and	"
						+ " [WhiteWeight]>0 AND WrapperNumber='WR6' "
						+ " group by "
						+ "  [DateEntered],[WrapperNumber],[GradeCode],[CustomerGradeCode],[Customer],[Diameter],[RollWidth],[CoreSize] "
				...
				List<WrapperProduction> wrapperProductions=jdbcTemplate.query(sql, new Object[]{
			...
			//Red Weight
			{
				String sql=" SELECT "
						...
						+ " [DateTimeEntered] between ? and ? "
						+ " and	"
						+ " [RedWeight]>0   AND WrapperNumber='WR6' "
						+ " group by "
						+ "  [DateEntered],[WrapperNumber],[GradeCode],[CustomerGradeCode],[Customer],[Diameter],[RollWidth],[CoreSize] "
						+ " Order by [DateEntered],[Customer]";
				System.out.println(sql);
	...
	public Map<String, Object> isNewRedEntryExist() {
		...
		String sql="SELECT count(*) FROM [tblWrapperProduction] where ( [RedWeight]>0 or  [RejectWeight]>0)  and WrapperNumber='WR6' and WrapperNumber<>'ZZZ' and [DateTimeEntered] between ? and ?";
		...
		String sql2="SELECT count(*) FROM [tblWrapperProduction] where ( [RedWeight]>0 or  [RejectWeight]>0)  and WrapperNumber='WR6' and WrapperNumber<>'ZZZ' and [DateTimeEntered] between ? and ? ";
		
		int count2=0;
		try {
			count2=jdbcTemplate.queryForObject(sql2,new Object[]{
	 ...
	 */
	@Override
	public List<WrapperProduction> getWrapperRedRejectProductionData(Date sdate,
			Date edate) {
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSourceP);
		String sql="SELECT * FROM [tblWrapperProduction] where ( [RedWeight]>0 or  [RejectWeight]>0)  and [DateTimeEntered] between ? and ? and WrapperNumber='WR6'  order by [DateTimeEntered]";
		List<WrapperProduction> productions=jdbcTemplate.query(sql, new Object[]{
				new Timestamp(sdate.getTime()),
				new Timestamp(edate.getTime())
		}, new RowMapper<WrapperProduction>(){
...
/*				production.setQualityHold(rs.getBoolean("QualityHold"));
	 ...
	 */
	@Override
	public WrapperProduction getWrapperTonsOfShift() {
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSourceP);
	 ...
	 */
	@Override
	public List<WrapperProduction> getWrapperAvgByGrade(Date sdate, Date edate) {
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSourceP);
...
*/		
		...
		String sql="SELECT GradeCode,Diameter,RollWidth,avg(WhiteWeight) as WhiteWeightAvg,max(WhiteWeight) as WhiteWeightMax,min(WhiteWeight) as WhiteWeightMin"
				+ " FROM tblWrapperProduction "
				+ " where DateTimeEntered between ? and ? and WhiteWeight>0 AND WrapperNumber='WR6' "
				+ " group by GradeCode,Diameter,RollWidth ";
	 ...
	 */
	@Override
	public List<WrapperProduction> getWrapperDataByDate(Date sdate, Date edate) throws ProductionException {
		...
		List<WrapperProduction> productions=new ArrayList<WrapperProduction>();
		try {
			 productions=jdbcTemplate.query(sql, paramSource, new RowMapper<WrapperProduction>() {
	 ...
	 */
	@Override
	public List<WrapperProduction> getWrapperProductionsForRollBreak(Date sdate,
		...
		String sql="";
		List<WrapperProduction> wrapperProductions=null;
		List<WrapperProduction> wrapperProductions1=null;
		...
		if(breakSize>1){
			if(shift.equalsIgnoreCase("Day")|| shift.equalsIgnoreCase("Night")){
				sql="SELECT COUNT(*) As [Total_Rolls_Produce],a.[Shift],b.[Total_Rolls_With_Breaks]  "
						...
						+ "Where ([DateTimeEntered] between ? and ? )  "
						+ "AND [WrapperNumber] <> 'ZZZ' AND [Breaks]>0 and [Shift]=? AND [WrapperNumber]='WR6' "
						+ "Group By [tblWrapperProduction].[Shift] ) as b on b.Shift=a.Shift "
						+ " Where (a.[DateTimeEntered] between ? and ? ) "
						+ "AND a.[WrapperNumber] <> 'ZZZ'  AND [WrapperNumber]='WR6' "
					...
					sql="SELECT COUNT(*) As [Total_Rolls_Produce],a.[Shift],b.[Total_Rolls_With_Breaks]  "
							...
							+ "FROM [production].[dbo].[tblWrapperProduction]  "
							+ "Where ([DateTimeEntered] between ? and ? )  "
							+ "AND [WrapperNumber] <> 'ZZZ' AND [Breaks]>0  AND [WrapperNumber]='WR6' "
							+ "Group By [tblWrapperProduction].[Shift] ) as b on b.Shift=a.Shift "
							+ " Where (a.[DateTimeEntered] between ? and ? ) "
							+ "AND a.[WrapperNumber] <> 'ZZZ'   AND [WrapperNumber]='WR6'"
							+ "Group By a.[Shift],b.[Total_Rolls_With_Breaks] ";
						...
						new RowMapper<WrapperProduction>(){
							...
							public WrapperProduction mapRow(ResultSet rs, int arg1)
								...
								if(totalRoll>0){
									...
									Integer obj1 = Integer.valueOf(totalRollBreaks);
									Integer obj2 =Integer.valueOf(totalRollProduse);
			...
			if(shift.equalsIgnoreCase("Day")|| shift.equalsIgnoreCase("Night")){
				sql="SELECT COUNT(*) As [Total_Rolls_Produce],a.[Shift] "
						+ " FROM [production].[dbo].[tblWrapperProduction] a  "
						+ "Where (a.[DateTimeEntered] between ? and ? )  "
						+ "AND a.[WrapperNumber] <> 'ZZZ'  and [Shift]=?  AND [WrapperNumber]='WR6'"
						+ "Group By a.[Shift] ";
						param=new Object[]{
							new Timestamp(sdate.getTime()),
							new Timestamp(edate.getTime()),
			...
			try {
				...
				jdbcTemplate1.update(sql1, new Object[] {});
			...
			for (final WrapperProduction data : wrapperProductions) {
				...
				jdbcTemplate2.update(new PreparedStatementCreator() {
			...
			if(shift.equalsIgnoreCase("Day")|| shift.equalsIgnoreCase("Night")){
				sql1="SELECT COUNT(*) As [Total_Rolls_With_Breaks],[Shift]  "
						+ "FROM [production].[dbo].[tblWrapperProduction]  "
						+ "Where ([DateTimeEntered] between ? and ? )  "
						+ "AND [WrapperNumber] <> 'ZZZ' AND Shift=? AND [Breaks]>0  AND [WrapperNumber]='WR6' Group By [tblWrapperProduction].[Shift] ";
						param=new Object[]{
							new Timestamp(sdate.getTime()),
							new Timestamp(edate.getTime()),
							shift
					...
					sql1="SELECT COUNT(*) As [Total_Rolls_With_Breaks],[Shift]  "
							+ "FROM [production].[dbo].[tblWrapperProduction]  "
							+ "Where ([DateTimeEntered] between ? and ? )  "
							+ "AND [WrapperNumber] <> 'ZZZ'  AND [Breaks]>0  AND [WrapperNumber]='WR6' Group By [tblWrapperProduction].[Shift] ";
							param=new Object[]{
								new Timestamp(sdate.getTime()),
								new Timestamp(edate.getTime())
							};
			...
			try {
				wrapperProductions1=jdbcTemplate.query(sql1,
						param, 
						new RowMapper<WrapperProduction>(){
			...
			for (final WrapperProduction data1 : wrapperProductions1) {
				...
				String sql3="update [tblWrappedSheetBreak_pm6] SET "
						...
						jdbcTemplate3.execute(sql3, new PreparedStatementCallback<Boolean>() {
			...
			try {
				wrapperProductions=jdbcTemplate4.query(sql4, 
						new RowMapper<WrapperProduction>(){
	 ...
	 */
	@Override
	public List<WrapperProduction> checkBreaks(Date sdate, Date edate,String shift) {
	 ...
	 */
	@Override
	public List<WrapperProduction> getWrapperRedRejectProductionData_PM5(
