 ...
 */
@Component
public class MachineProductionSummaryDaoImp implements MachineProductionSummaryDao{
	...
	public List<MachineProduction> getMachineBreakFreeProduction(Date sdate,
		...
		List<MachineProduction> details=new ArrayList<MachineProduction>();
			...
			try{
				...
				if(shift==""){
					sql="SELECT COUNT(*) As [Total_Rolls_Produce],a.[Shift],b.[Total_Rolls_With_Breaks],b.[RollID],b.[GradeCode]  FROM [production].[dbo].[tblMachineProduction] a "
							...
							+ "Where ([DateTimeEntered] between ? and ? ) AND [MachineNumber] <> 'ZZZ' AND [NumberBreaks]>0  AND [MachineNumber]='TM6'"
							+ "Group By [tblMachineProduction].[Shift],[RollID],[GradeCode] ) as b on b.Shift=a.Shift "
							+ "Where (a.[DateTimeEntered] between ? and ? ) AND a.[MachineNumber] <> 'ZZZ'  AND [MachineNumber]='TM6'"
					...
					sql="SELECT COUNT(*) As [Total_Rolls_Produce],a.[Shift],b.[Total_Rolls_With_Breaks],b.[RollID],b.[GradeCode]  FROM [production].[dbo].[tblMachineProduction] a "
							+ "INNER JOIN ( "
							+ "SELECT COUNT(*) As [Total_Rolls_With_Breaks],[Shift],[RollID],[GradeCode] FROM [production].[dbo].[tblMachineProduction]  "
							+ "Where ([DateTimeEntered] between ? and ? ) AND [MachineNumber] <> 'ZZZ' AND [NumberBreaks]>0 And [Shift]=? AND [MachineNumber]='TM6' "
							+ "Group By [tblMachineProduction].[Shift],[RollID],[GradeCode] ) as b on b.Shift=a.Shift "
							+ "Where (a.[DateTimeEntered] between ? and ? ) AND a.[MachineNumber] <> 'ZZZ' AND [MachineNumber]='TM6' "
							+ "Group By a.[Shift],b.[Total_Rolls_With_Breaks],b.[RollID],b.[GradeCode]";
					maps=jdbcTemplate.queryForList(sql, new Object[]{
				...
				if(maps.size()>1){
					for (Map<String, Object> map : maps) {
						final MachineProduction data=new MachineProduction();
						...
						Integer obj1 = Integer.valueOf(Total_Rolls_With_Breaks);
						Integer obj2 = Integer.valueOf(Total_Rolls_Produce);
