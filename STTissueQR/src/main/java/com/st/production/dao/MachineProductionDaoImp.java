 ...
 */
@Repository
public class MachineProductionDaoImp implements MachineProductionDao {
	...
	public List<MachineProduction> getMachineProductions(Date sdate, Date edate) throws ProductionException {
		...
		String sql="select "
				...
				+ " or "	
				+ "[DateTimeEntered] between ? and ? ) AND  [MachineNumber]='TM6'"
				+ " order by [DateTimeEntered]";
		
		List<MachineProduction> machineProductions=null;
	...
	//	System.out.println(sql);
		...
		try {
			...
			}, new RowMapper<MachineProduction>() {
	 ...
	 */
	@Override
	public List<MachineProduction> getMachineProductionsByMonth(Date sdate,
	 ...
	 */
	@Override
	public List<MachineProduction> getMachineProductions(Date sdate,
	...
	//	System.out.println(edate);
		...
		String sql="select "
				...
				+ "( [DateEntered] between ? and ? "
				+ " OR "
				+ "[TimeEntered] between ? and ? ) AND [MachineNumber]='TM6' ";
		...
		try {
			machineProductions=jdbcTemplate.query(sql, param, new RowMapper<MachineProduction>() {
...
final JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSourceP);
		...
		//	System.out.println(ecal.getTime());
			...
			try {
				String sql="select m.[DateEntered],max(m.[MachineSpeed]) as MachineSpeed,"
						...
						+ " And  (g.GradeCode=m.GradeCode)   and m.MachineNumber='TM6' "
			...
			//	System.out.println(sql);
				...
				List<MachineProduction> machineProductions=jdbcTemplate.query(sql, new Object[]{
	...
	public List<MachineAndWrapper> getMachineAndWrapper(Date sdate, Date edate) throws ProductionException {
		...
		String sql=DatabaseUtil.getSQL("sql/MachineVsWrapper1.sql");

		
		List<MachineAndWrapper> machineAndWrappers=new ArrayList<MachineAndWrapper>();
		try {
			machineAndWrappers=jdbcTemplate.query(sql, parameterSource, new RowMapper<MachineAndWrapper>(){

				@Override
				public MachineAndWrapper mapRow(ResultSet rs, int arg1)
						throws SQLException {
					MachineAndWrapper machineAndWrapper=new MachineAndWrapper();
					...
					List<MachineAndWrapper> wrappers=jdbcTemplate.query(sql, parameterSource, new RowMapper<MachineAndWrapper>() {

						@Override
						public MachineAndWrapper mapRow(ResultSet rs, int arg1)
								throws SQLException {
							MachineAndWrapper wrapper=new MachineAndWrapper();
					...
					if(wrappers.size()>0){
						MachineAndWrapper wrapper=wrappers.get(0);
	 ...
	 */
	@Override
	public List<MachineProduction> getMachineProductionsForRollBreak(Date sdate,
		...
		if(brakSize>0){
			if(shift.equalsIgnoreCase("Day")|| shift.equalsIgnoreCase("Night")){
				sql="SELECT COUNT(*) As [Total_Rolls_Produce],a.[Shift],b.[Total_Rolls_With_Breaks]   "
						...
						+ "AND [MachineNumber] <> 'ZZZ' AND [NumberBreaks]>0 and [Shift]=? AND [MachineNumber]='TM6' "
						+ "Group By [tblMachineProduction].[Shift] "
						+ ") as b on b.Shift=a.Shift "
						+ "Where (a.[DateTimeEntered] between ? and ? )  "
						+ "AND a.[MachineNumber] <> 'ZZZ'   AND [MachineNumber]='TM6'  "
				...
				sql="SELECT COUNT(*) As [Total_Rolls_Produce],a.[Shift],b.[Total_Rolls_With_Breaks]  "
						...
						+ "FROM [production].[dbo].[tblMachineProduction]  "
						+ "Where ([DateTimeEntered] between ? and ? )  "
						+ "AND [MachineNumber] <> 'ZZZ' AND [NumberBreaks]>0 AND [MachineNumber]='TM6'  "
						+ "Group By [tblMachineProduction].[Shift] "
						+ ") as b on b.Shift=a.Shift "
						+ "Where (a.[DateTimeEntered] between ? and ? )  "
						+ "AND a.[MachineNumber] <> 'ZZZ'    AND [MachineNumber]='TM6' "
			...
			try{
				...
				machineProductions=jdbcTemplate.query(sql,
						...
						new RowMapper<MachineProduction>(){
							...
							@Override
							public MachineProduction mapRow(ResultSet rs, int arg1)
									throws SQLException {
								
								MachineProduction macineProduction=new MachineProduction();
			...
			if(shift.equalsIgnoreCase("Day")|| shift.equalsIgnoreCase("Night")){
				sql="SELECT COUNT(*) As [Total_Rolls_Produce],a.[Shift] "
						+ "FROM [production].[dbo].[tblMachineProduction] a "
						+ "Where (a.[DateTimeEntered] between ? and ? )   "
						+ "AND a.[MachineNumber] <> 'ZZZ'   and [Shift]=? AND [MachineNumber]='TM6' "
						+ "Group By a.[Shift]";
						param=new Object[]{
							new Timestamp(sdate.getTime()),
							new Timestamp(edate.getTime()),
				...
				machineProductions=jdbcTemplate.query(sql,
						...
						new RowMapper<MachineProduction>(){
							...
							public MachineProduction mapRow(ResultSet rs, int arg1)
								...
								MachineProduction wrapperProduction=new MachineProduction();
	 ...
	 */
	@Override
	public List<MachineProduction> checkMachineProductionsForRollBreak(
		...
		String sql;
			...
			List<MachineProduction> wrapperProductions=null;
			try {
