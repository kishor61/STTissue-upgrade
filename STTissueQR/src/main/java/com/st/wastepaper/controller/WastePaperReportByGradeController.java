...
import com.st.wastepaper.report.WastePaperByGradeReportHandler;
 ...
 */
@Controller
@RequestMapping("/wastepaperreportbygrade")
public class WastePaperReportByGradeController {
	...
	@Autowired
	private WastePaperByGradeReportHandler wastePaperByGradeReportHandler;
	...
	public String main(HttpServletRequest request,HttpServletResponse response,Model model){
		...
		return "wastepaperreportbygrade";
	}
	@RequestMapping("/load")
	public String reportbyvendor(@RequestParam("sdate")String sdate,
		...
		List<WastepaperDetail> details=wastepaperreportbygradeservice.load(sDate,eDate,gradeid);
		...
		for(WastepaperDetail wp:details){
			...
			if(gradeid==666){
				wastepaperDetail.setItemDescription("");
			}else{
				itemDesciption=wp.getItemDescription();
				wastepaperDetail.setItemDescription(itemDesciption);
			}
	...
	public void export(@RequestParam("startdate")String sdate,
		...
		response.setHeader("Content-Disposition", "attachment; filename=Waste_Paper_Report_By_Grade.xlsx");
		File file=new File(context.getRealPath("/")+"WEB-INF/Waste_Paper_Report_By_Grade.xlsx");
		
		wastePaperByGradeReportHandler.getByGradeReport(details,file,response.getOutputStream());
		}
}
