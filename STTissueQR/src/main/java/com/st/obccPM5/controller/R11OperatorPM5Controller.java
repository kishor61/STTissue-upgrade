 ...
 */
@Controller
@RequestMapping("/r1operatorpm5")
public class R11OperatorPM5Controller {
	...
	public String operatorBasicchecklistView(
			...
			data.setCrew(crew);		
		try {
			R1OperatorPM5 dt = r1OperatorPM5Service.getOperatorData(position,shift,date, crew);
			if (dt == null) {
				data.setEdate(date);
				data.setShift(shift);
				data.setPosition(position);
	...
	public String R1OperatorPM5SaveData(@ModelAttribute("data") R1OperatorPM5 data,Model model, RedirectAttributes redirectAttributes) {
		
		
		model.addAttribute("data",data);
		 
		r1OperatorPM5Service.saveorUpdate(data);
