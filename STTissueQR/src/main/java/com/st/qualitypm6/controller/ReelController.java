...
@Controller
@RequestMapping(value="/reel")
public class ReelController {
	
	private static Logger logger=Logger.getLogger(ReelController.class);
	...
	public String add(Model model){
		...
		model.addAttribute("maxReel", reelService.getMaxReel());
	...
	public String add(@PathVariable("gradeCode")String gradeCode,
		...
		CommonUtil.save(CommonUtil.REEL_TEST_ENTRY_PAGE, request.getRequestURI());
		...
		Map<String, Object> objective=GradeTargetUtil.format(gradeTargets);
		...
		List<Reel> reels=reelService.getReels(gradeCode, date);
	...
	public String addBack(@PathVariable("date")String date,@PathVariable("gradeCode")String gradeCode,
		...
		List<Reel> reels=reelService.getReels(gradeCode, jdate);
...
//		Code Ends here Done By Roshan Tailor  
		...
		List<Reel> reelUpdateList=new ArrayList<Reel>();
...
//				Code Ends Here Done By Roshan Tailor
		...
		try {
			...
			if(reelUpdateList.size()==1){
				Reel reel=reelUpdateList.get(0);
				
				boolean checkFlag=true;
				//Reel Time
				Map<String, Date> reelMap=reelService.getPrevNextReelDate(reel.getReel());
				...
				if(checkFlag){
					Reel reelOld=reelService.getReelById(reel.getId());
					...
					if(reelOld.getReel().equals(reel.getReel())){
						...
						boolean flag=reelService.isReelNumberExist(reel.getReel(),yer);
						if(flag){
							map.put("flag", false);
					...
					map.put("error", "Previous reel no. "+(NumberUtils.toInt(reel.getReel(),0)-1)+" was entered at "+dateTimeFormat.format(reelMap.get(CommonProperties.REEL_PREV))+"."
...
//				Code Ends Here Done By Roshan Tailor
	...
	public void validateReelNumber(HttpServletRequest request,HttpServletResponse response){
		...
		boolean flag=reelService.isReelNumberExist(value);
	...
	public String getReelInfo(HttpServletRequest request,Model model) throws IOException{
		...
		List<Reel> reels=reelService.getReelByReelNo(reelNo);
	...
	public String findMatchGrade(HttpServletRequest request,
			Model model ){
		...
		List<Map<String,Object>> result=reelService.findMatch(reel);
		
		model.addAttribute("result", result);

		return "reelFindMatch";
	}
...
//		System.out.println("CurrentReelNumberByAajx:::::"+CurrentReelNumberByAajx);
//		System.out.println("CurrentGradeCodeByAjax:::::"+CurrentGradeCodeByAjax);
		int i = Integer.parseInt(CurrentReelNumberByAajx);
		String CurrentReelDB=reelService.getcurrentreel(CurrentReelNumberByAajx,CurrentGradeCodeByAjax);
		String LastRecordedGradeCode=reelService.getlastgradecode();
//		System.out.println("Hello Roshan Current Reel Number By DataBase Is::"+CurrentReelDB);
//		System.out.println("Hello Roshan Last  Grade Code By DataBase Is::"+LastRecordedGradeCode);
		...
		if(((reeldiffrence==0) && !(CurrentGradeCodeByAjax.equals(LastRecordedGradeCode))) || ((reeldiffrence==0) && (CurrentGradeCodeByAjax.equals(LastRecordedGradeCode))))
		{
			map.put("doentry", "move");
			String date=dateFormat.format(new Date());
			map.put("date", date);
...
//	Code Ends Here Done By Roshan Tailor
	...
	public void getBrightnessAvg(HttpServletRequest request,HttpServletResponse response) throws IOException {
		...
		//FRP Brightness average
		 double machBrightnessAvg=reelService.getBirghtnessAvgOfLastDay(grade);
