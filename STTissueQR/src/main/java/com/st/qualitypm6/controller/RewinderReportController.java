...
@Controller
@RequestMapping(value="/rewindreport")
public class RewinderReportController {
...
//				Code Ends Here Done By Roshan Tailor 
	...
	public String rewindReportView(@RequestParam("fromDate")String fromDate,
		...
		try {
			List<Rewind> rewinds = rewindService.getRewindData(
					dateFormat1.parse(fromDate), 
					dateFormat1.parse(toDate),grade,customer,reel,type);
			
			List<Map<String, Object>> datas =rewindService.getFormatedData(rewinds);
	...
	public void rewindReportExport(@RequestParam("fromDate")String fromDate,
		...
		try {
			...
			File file=new File(context.getRealPath("/")+"WEB-INF/RewinderReport.xls");
	...
	public void rewindReportExportPdf(@RequestParam("fromDate")String fromDate,
		...
		response.setHeader("Content-Disposition", "attachment; filename=ST Tisue_RewindlReport.pdf");
		qualityReportHandler.createRewindReport(rewinds, response.getOutputStream());
