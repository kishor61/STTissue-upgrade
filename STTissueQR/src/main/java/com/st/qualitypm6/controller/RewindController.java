...
@Controller
@RequestMapping(value = "/rewind")
public class RewindController {

	private static Logger logger = Logger.getLogger(RewindController.class);
	...
	public String add(@PathVariable("gradeCode") String gradeCode,
		...
		model.addAttribute("grades", gradeService.getGrades());
		model.addAttribute("date", dateFormat1.format(new Date()));
		model.addAttribute("reels", reelService.getReelByGradeCode(gradeCode));
	...
	public String add(@PathVariable("gradeCode") String gradeCode,
		...
		List<Rewind> rewinds = rewindService.getRewinds(gradeCode, shiftDate);
		...
		if (custName != null && custName.trim().equals("")) {
			...
			model.addAttribute("custName",
					reelService.getCustomerName(gradeCode, reelNo));
		}
	...
	public String addBack(@PathVariable("date") String date,
		...
		if (jdate != null) {
			...
			rewinds = rewindService.getRewinds(gradeCode, jdate);
		...
		if (custName != null && custName.trim().equals("")) {
			...
			model.addAttribute("custName",reelService.getCustomerName(gradeCode, reelNo));
		}
...
//		Code Ends Here Done By Roshan Tailor
		...
		List<Rewind> rewindsList = new ArrayList<Rewind>();
		List<Rewind> rewindsUpdateList = new ArrayList<Rewind>();
...
//				Code Ends Here Done By Roshan Tailor
		...
		try {
			...
			/*if(intIds.size()>0){
				rewindService.updateEntryById(intIds, enterAutoFlag);
			}
			*/
			...
			if (rewindsUpdateList.size() == 1) {
				Rewind rewind = rewindsUpdateList.get(0);
				Map<String, Object> map = new HashMap<>();
				Rewind rewindOld = rewindService.getRewindById(rewind.getId());

				if (rewind.getSetNo().equalsIgnoreCase(rewindOld.getSetNo())) {
					map.put("flag", true);
					rewindService.update(rewind);
					map.put("id", rewind.getId());
				} else {
					boolean flag = rewindService.isSetNumberExist(rewind.getSetNo());
					if (flag) {
						map.put("flag", false);
					} else {
						map.put("flag", true);
			...
			if (rewindsList.size() == 1) {
				Rewind rewind = rewindsList.get(0);
				Map<String, Object> map = new HashMap<>();
				...
				if (flag) {
					...
					map.put("flag", true);
					int id = rewindService.save(rewind);
					map.put("id", id);
				}
				response.getWriter().write(new Gson().toJson(map));
			}
...
//				 System.out.println("Customer Name:-"+customer);
		...
		try {
			for (Rewind rewind : rewindsUpdateList) {
//				All Values Are Coming rewind Object
	...
	public void validateReelNumber(HttpServletRequest request,
		...
		boolean flag = rewindService.isSetNumberExist(setNo);
	...
	public String rewindInfo(HttpServletRequest request,Model model) {
		...
		if(StringUtils.isNotBlank(reel) || StringUtils.isNotBlank(set)){
			rewinds=rewindService.getRewindInfo(reel,set);
		}
