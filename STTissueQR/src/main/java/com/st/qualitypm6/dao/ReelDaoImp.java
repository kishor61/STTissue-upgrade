...
public class ReelDaoImp implements ReelDao {
	...
	//	System.out.println("End Date="+ecal.getTime());
		...
		String sql="select * from [reelTesting] where"
				+ " [GradeCode]=?"
				+ " and "
				+ " [Date] between ? and ?  "
				+ " order by [Date], [Reel], [ID]";
		
		List<Reel> reels = jdbcTemplate.query(sql, new Object[] { gradeCode,
				new Timestamp(scal.getTime().getTime()),
				new Timestamp(ecal.getTime().getTime()) }, new ReelMapper());
	...
	public boolean isReelNumberExist(String reelNo,String year) {
		...
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
		String sql="select count(*) from [reelTesting] where [Reel]='"+reelNo+"' and year(date)='"+year+"'";
	...
	public boolean isReelNumberExist(String reelNo) {
		...
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
		String sql="select count(*) from [reelTesting] where [Reel]='"+reelNo+"'";
	...
	public List<String> getReels() {
		...
		String sql = "select [reel] from [reelTesting] order by [Date] DESC,[Reel] DESC, [ID]  DESC";
	...
	public List<String> getReelByGradeCode(String gradeCode) {
		...
		String sql = "select [reel] from [reelTesting] where [GradeCode]='"+gradeCode+"'   order by [Date], [Reel], [ID]";
	...
	public void delete(final List<String> idsList) {
		...
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
		String sql="delete from [reelTesting] where [id]=?";
	...
	public Object getCustomerName(String gradeCode, String reelNo) {
		...
		String custName="";
		String sql="select [Customer] from [reelTesting] where [GradeCode]=? and [Reel]=?";
		try{
	...
	@Override
	public int save(final Reel reel) throws Exception {
    JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
		
		final String sql="insert into [reelTesting]"
				+ "("
				+ "[Date],"
				+ "[Time],"
				+ "[GradeCode],"
...
//						Code Ends Here Done By Roshan Tailor
						statement.setString(25, reel.getCustomer());
						statement.setString(26, reel.getRemarks());
						statement.setString(27, reel.getCustomer1());
						statement.setString(28, reel.getCustomer2());
...
@Override
	public void update(final Reel reel) throws Exception {
		String sql="update [reelTesting] SET "
...
//							Code Ends Here Done By Roshan Tailor
					...
					jdbcTemplate.execute(sql, new PreparedStatementCallback<Boolean>() {
						...
						public Boolean doInPreparedStatement(
								...
								DataAccessException {
							statement.setTimestamp(1, new Timestamp(reel.getDate().getTime()));
							System.out.println("Reel Call in Dao Impl ::"+reel.getId());
							System.out.println("Reel Call in Dao Impl::"+reel);
							if(reel.getTime()!=null){
								statement.setTimestamp(2, new Timestamp(reel.getTime().getTime()));
							}else{
								statement.setNull(2, Types.TIMESTAMP);
...
//							Code Ends Here Done By Roshan Tailor						
							...
							statement.setInt(33, reel.getId());
   ...
   /** // Assuming 'dataSource' is properly initialized somewhere in your class
    private JdbcTemplate jdbcTemplate;

    // Constructor to initialize JdbcTemplate
    public ReelDaoImp(DataSource dataSource) {
    ...
    public void update(final Reel reel) throws Exception {
        ...
        // Logging to verify the ID being updated
        System.out.println("Updating reel with ID: " + reel.getId());

        // Using jdbcTemplate.update() in a non-static context
        int rowsAffected = jdbcTemplate.update(sql, new PreparedStatementSetter() {
            @Override
            public void setValues(PreparedStatement ps) throws SQLException {
                ps.setTimestamp(1, new Timestamp(reel.getDate().getTime()));

                // Handling Time field nullability
                if (reel.getTime() != null) {
                    ps.setTimestamp(2, new Timestamp(reel.getTime().getTime()));
                } else {
                    ps.setNull(2, Types.TIMESTAMP);
                }

                ps.setString(3, reel.getGradeCode());
                ps.setString(4, reel.getReel());
                ps.setDouble(5, reel.getScannerBasisWt());
                ps.setDouble(6, reel.getActualBasisWt());
                ps.setDouble(7, reel.getBulk());
                ps.setDouble(8, reel.getMdTensile());
                ps.setDouble(9, reel.getCdTensile());
                ps.setDouble(10, reel.getMdStretch());
                ps.setDouble(11, reel.getMdcdTensileRatio());
                ps.setDouble(12, reel.getMdWetTensile());
                ps.setDouble(13, reel.getCdWetTensile());
                ps.setDouble(14, reel.getWetDryRatio());
                ps.setDouble(15, reel.getBrightness());
                ps.setDouble(16, reel.getFwaFlow());
                ps.setDouble(17, reel.getDirtCount());
                ps.setDouble(18, reel.getFwaDosage());
                ps.setDouble(19, reel.getTph());
                ps.setDouble(20, reel.getTrim());
                ps.setDouble(21, reel.getCrepeRatio());
                ps.setDouble(22, reel.getLvalue());
                ps.setDouble(23, reel.getAvalue());
                ps.setDouble(24, reel.getBvalue());
                ps.setDouble(25, reel.getDeltae());
                ps.setString(26, reel.getCustomer());
                ps.setString(27, reel.getRemarks());
                ps.setDouble(28, reel.getAkd());
                ps.setDouble(29, reel.getWetstrength());
                ps.setDouble(30, reel.getDrystrengthflow());
                ps.setString(31, reel.getCustomer1());
                ps.setString(32, reel.getCustomer2());
                ps.setInt(33, reel.getId());  // ID is being passed here
            }
        });

        // Logging the number of rows updated
        System.out.println("Rows updated: " + rowsAffected);

        // If no rows are affected, check if the ID exists in the database
        if (rowsAffected == 0) {
            System.out.println("No rows were updated. Please verify if the ID exists in the database.");
        }
    }/**
...
*/
	...
	public Reel getReelById(int id) {
		...
		String sql="select * from [reelTesting] where [ID]=?";
		Reel reel=null;
		try {
			reel=jdbcTemplate.queryForObject(sql, new Object[]{id}, new ReelMapper());
	...
	public List<Reel> getReelByReelNo(String reelNo) {
		...
		String sql="select * from [reelTesting] where [Reel]=?";
		List<Reel> reels=null;
		try {
			reels=jdbcTemplate.query(sql, new Object[]{reelNo}, new ReelMapper());
	...
	public List<Map<String, Object>> findMatch(Reel reel) {
		...
		if(gradeCode!=null && !gradeCode.trim().equals("")){
			...
			String sql1=null;
			if(gc_2.equals("98")){
				sql1="select [GradeCode] from [Grade] where [GradeCode] like '%-98-%'";
			}else{
				sql1="select [GradeCode] from [Grade] where [GradeCode] not like '%-98-%'";
			}
	...
	public Map<String, Date> getPrevNextReelDate(String reel) {
		...
		// System.out.println("nextReel "+nextReel);
		 ...
		 String sql="select [Date] from [reelTesting] where [Reel]=?";
	...
	public List<String> getGrades(Date sdate, Date edate, String customer) {
		...
		String sql="select distinct[GradeCode] from [reelTesting] "
				+ " where "
				+ " [Customer]=? "
				+ " and "
				+ " [Date] between ? and ? ";
...
//				+"	FROM [reelTesting])  order by ID desc";
		...
		String sql="select TOP 1 [Reel] from [reelTesting] where ((reelTesting.[GradeCode])=?)"
				+ " order by ID desc";
		try {
...
//	Code Starts From Here Done By Roshan Tailor Date :- 04/16/2015
	...
	public String getlastgradecode() {
		...
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
		String sql="select TOP 1 [GradeCode] from [reelTesting] order by ID desc";
		try{
...
//	Code Ends Here Done By Roshan Tailor
	...
	public List<Reel> getReelData(Date sdate, Date edate, String grade,
		...
		String sql="select * from [reelTesting] where ([Date] between :sdate and :edate) ";
		...
		List<Reel> reels=null;
		try {
			reels=jdbcTemplate.query(sql, parameterSource, new ReelMapper());
	...
	/* (non-Javadoc)
	 * @see com.st.qualitypm6.dao.ReelDao#getReelByReelNo(java.lang.String, java.util.Date, java.util.Date)
	 */
	@Override
	public List<Reel> getReelByReelNo(String reelNo, Date dateFrom, Date dateTo) {
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
		MapSqlParameterSource parameterSource=new MapSqlParameterSource();
		...
		String sql="select * from [reelTesting] where  [Date] between ? and ? and [Reel]=?";
		List<Reel> reels=null;
		try {
			reels=jdbcTemplate.query(sql, new Object[]{new Timestamp(scal.getTime().getTime()),new Timestamp(ecal.getTime().getTime()),reelNo,}, new ReelMapper());
	...
	/* (non-Javadoc)
	 * @see com.st.qualitypm6.dao.ReelDao#getReelByReelNopm5(java.lang.String, java.util.Date, java.util.Date)
	 */
	@Override
	public List<Reel> getReelByReelNopm5(String reelNo, Date dateFrom, Date dateTo) {
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
		MapSqlParameterSource parameterSource=new MapSqlParameterSource();
		...
		String sql="select * from [reelTesting_pm5] where  [Date] between ? and ? and [Reel]=?";
		List<Reel> reels=null;
		try {
			reels=jdbcTemplate.query(sql, new Object[]{new Timestamp(scal.getTime().getTime()),new Timestamp(ecal.getTime().getTime()),reelNo,}, new ReelMapper());
	...
	/* (non-Javadoc)
	 * @see com.st.qualitypm6.dao.ReelDao#getReelByReelNo_Pm5(java.lang.String, java.util.Date, java.util.Date)
	 */
	@Override
	public List<Reel> getReelByReelNo_Pm5(String reelNo, Date dateTo, Date  dateFrom) {
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
		MapSqlParameterSource parameterSource=new MapSqlParameterSource();
