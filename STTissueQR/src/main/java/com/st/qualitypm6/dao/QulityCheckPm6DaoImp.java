 ...
 */
@Service
public class QulityCheckPm6DaoImp implements QulityCheckPm6Dao {
	...
	public List<QulityCheck_Pm6> getDataOfQulityCheck_Pm6(Date date) {
      ...
      JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
		...
		String sql="select * from tbl_QulityCheck_Pm6 where date between ? and ?  order by date ";   
...
JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
		...
		final String sql="insert into [tbl_QulityCheck_Pm6]"
				+ "("
				+"[time],"
				+"[date],"
				+"[gradecode],"
		...
		jdbcTemplate.update(new PreparedStatementCreator() {
			...
			//	statement.setTimestamp(2, new Timestamp(wm.getDate().getTime())); frpproductionopdataentry
	...
	/* (non-Javadoc)
	 * @see com.st.qualitypm6.dao.QulityCheckPm6Dao#qualitychecklistdelete(int) KISH020493
	 */
	@Override
	public void qualitychecklistdelete(int id) {
		JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
		String sql="delete from [tbl_QulityCheck_Pm6] where [ID]=?";
	...
	public void updatequalitychecklist(final QulityCheck_Pm6 wm) {
		 ...
		 */
				JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
				final String sql="update [tbl_QulityCheck_Pm6] set "
						+"[time]=?,"
						+"[date]=?,"
						+"[gradecode]=?,"
						+"[customer]=?,"
...
JdbcTemplate jdbcTemplate=new JdbcTemplate(dataSource);
		...
		String sql="select * from tbl_QulityCheck_Pm6 where [id]=?";
